...
Using the DBF, feasibility may be examined by checking whether the demand over any interval of length $\delta$ exceeds processor availability \cite{baruah_preemptively_1990}.
...
Other feasibility results exist for different scheduling settings such as nonpreemptive tasks \cite{ekelin_clairvoyant_2006}, multiple processors \cite{baker_multiprocessor_2003}, precedence constraints \cite{shi_scheduling_2006}, or a combination thereof.
Additionaly complications exists for systems where computation is performed on heterogeneous architectures (processors with different speeds, for example) or where certain tasks must be completed by specific processors (as opposed to any processor). 
...
To illustrate how tying physical dynamics to real-time parameters brings complexity, consider the typical sporadic task definition: $\tau = (a,p,c,d)$.
...
Even with the appropriate model for $x(t)$, we now need to calculate utilization or demand using a variable interarrival time or cannot properly characterize demand.
