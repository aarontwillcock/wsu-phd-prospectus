...
\begin{figure}
    ...
    % \textbf{Intake:} As the crank angle increases towards $\theta = 180 \deg$, the intake valve opens to allow air and fuel to enter the cylinder and mix.
    % At $\theta = 180\deg$, the fuel and air mixture is sealed inside the cylinder.
    % This point is known as Bottom Dead Center (BDC).
    % \textbf{Compression:} The crankshaft rotates up through $\theta = 360\deg = 0\deg$, compressing the fuel and air.
    % \textbf{Combustion:} Around $\theta = 0\deg$, the spark plug ignites the fuel and air mixture beginning combustion.
    % The crankshaft rotates back through $\theta = 180\deg$ as the expanding gas from combustion provides torque on the crankshaft.
    % \textbf{Exhaust:} As the crank angle rotates back up through $\theta = 0\deg$, the exhaust valve opens and the exhaust gas is forced out of the cylinder.
    \label{fig:Engine}
\end{figure}
...
\begin{definition}[$k$-Subsequence of $S$]\label{def:k-subsequence}\mbox{}
...
\begin{property}[Leading Injection Suboptimality]\label{prop:ascending-dominates-L}\mbox{}
...
\begin{property}[Internal Injection Suboptimality]\label{prop:ascending-dominates-I}\mbox{}
...
In addition, sequence $\omega_1, \ldots, \omega_{\ell-1}, \omega'_\ell$ and $\omega_1, \ldots, \omega_{\ell-1}, \omega_\ell$ have equivalent demand since the minimum time between $\omega_{\ell-1}$, $\omega'_\ell$ is reduced (Property~\ref{prop:neg-deriv-min-interarrival}) and the execution of this subsequence is equivalent since $c(\omega'_\ell) = c(\omega_\ell)$.  Furthermore, since the release of the $\ell^{\rm th}$ job occurs earlier in the new sequence and the relative deadline is decreased (Property~\ref{prop:deadline-derivative}), the deadline of job released at speed $\omega'_{\ell}$ will have its deadline in the interval $[t_a, t_b]$ if the job released at $\omega_\ell$ in the original sequence did.  Therefore, the new subsequence demand is no less than the original sequence demand.
...
\begin{table}
    ...
    \begin{tabular}{| r | l l l l l l | l |} 
        \hline
        $i^{\mathrm{th}}$ mode & 1 & 2 & 3 & 4 & 5 & 6 & \(\omega_{rb_m}\)\\ \hline\hline
        \textbf{\(\omega_{rb_i}\)} & 1200 & 2200 & 3200 & 4200 & 5200 & 6200 &\multirow{2}{*} {7200}\\
        \textbf{\(c(\omega_{rb_i})\)} & 965 & 576 & 424 & 343 & 277 & 246 & \\ \hline
    \end{tabular}
    \egroup
...
\begin{table}
    ...
    \caption{Runtime Comparison of DRT and Knapsack AVR Algorithm: DBF over 1$s$}
    \label{fig:Exp1Comp}
\end{table}
